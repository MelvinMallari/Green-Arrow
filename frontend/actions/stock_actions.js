...
export const fetchStockData = symbol => dispatch =>
  StockApiUtil.fetchStockData(symbol).then(stockData => dispatch(receiveStockData(symbol, stockData)));

export const fetchStockIntradayData = symbol => dispatch =>
  StockApiUtil.fetchStockIntradayData(symbol).then(stockIntraData =>
    dispatch(receiveStockIntradayData(symbol, stockIntraData))
  );

export const fetchStockInfo = symbol => dispatch =>
  StockApiUtil.fetchStockInfo(symbol).then(stockInfo => dispatch(receiveStockInfo(symbol, stockInfo)));
